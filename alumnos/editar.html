<!DOCTYPE html>
<html lang="es" ng-app="ejemplo5">
<head>
    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">

    <!-- Bootstrap CSS -->
    <link rel="stylesheet" href="../recursos/css/bootstrap.min.css">
    <script src="../recursos/js/angular.min.js"></script>
    <script src="../recursos/js/editarAController.js"></script>
</head>
<body ng-controller="ealumnoC">
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
    <a class="navbar-brand" href="#">Instituto de idomas</a>
    <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarSupportedContent"
            aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
        <span class="navbar-toggler-icon"></span>
    </button>

    <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <ul class="navbar-nav mr-auto">
            <li class="nav-item">
                <a class="nav-link" href="../index.html">Inicio <span class="sr-only">(current)</span></a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="../nosotros.html">Acerca de nosotros</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="../contacto.html">Contacto</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="../login.html">Login</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="../login.html">Registro</a>
            </li>
            <li class="nav-item dropdown">
                <a class="nav-link dropdown-toggle" id="navbarDropdownMenuLink" data-toggle="dropdown"
                   aria-haspopup="true" aria-expanded="false">
                    Panel de control
                </a>
                <div class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
                    <a class="dropdown-item" href="../alumnos/alumnos.html">Alumnos</a>
                    <a class="dropdown-item" href="profesores.html">Profesores</a>
                    <a class="dropdown-item" href="../cursos.html">Cursos</a>
                </div>
            </li>
        </ul>
    </div>
</nav>
<br>
<div class="container">
    <script>
        id = getParameterByName("id");
        document.write("<h4>Registro: " + id + "</h4>");
        //edit(id);

    </script>
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    <h4>Editgar</h4>
                    <form action="">
                        <div class="form-group">
                            <label>curso</label>
                            <input type="text" ng-model="curso" class="form-control">
                        </div>
                        <div class="form-group">
                            <label>profesor</label>
                            <input type="text" ng-model="profesor" class="form-control">
                        </div>
                        <div class="form-group">
                            <label>porcentaje</label>
                            <input type="text" ng-model="porcentaje" class="form-control">
                        </div>
                        <div class="form-group">
                            <label>aula</label>
                            <input type="text" ng-model="aula" class="form-control">
                        </div>
                        <div class="form-group">
                            <label>hora</label>
                            <input type="text" ng-model="hora" class="form-control">
                        </div>
                        <input type="submit" class="btn btn-primary">
                    </form>
                </div>
            </div>
        </div>
    </div>
    <br>
    <footer>
        <p>Instituto de Idiomas de México 2017</p>
    </footer>
</div>

<!-- Optional JavaScript -->
<!-- jQuery first, then Popper.js, then Bootstrap JS -->
<script src="../recursos/js/jquery-3.2.1.slim.min.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.11.0/umd/popper.min.js"
        integrity="sha384-b/U6ypiBEHpOf/4+1nzFpr53nxSS+GLCkfwBdFNTxtclqqenISfwAzpKaMNFNmj4"
        crossorigin="anonymous"></script>
<script src="../recursos/js/bootstrap.min.js"></script>
</body>
</html>